...
Our team was responsible for building a system to automate the input and retrieval of relevant data. We designed and implemented a system that accepts the current database and allows it to be modified and extended. The system has different features for the different privileges each type of users have.\\
An Administrator is able to : 
\begin{itemize} 
	\item Register new members and delete those who leave the program 
	\item Delete and add initiatives
	\item Search the database for particular parameters and organise the information 
	\item Export graphs and tables into csv
	\item Manage the fundings to particular initiative or a member	
	\item Access all the given personal information of all users and make changes if necessary 
	\item Keep track of the progress of both members and volunteers
	\item Preview the answers from the feedback received
\end{itemize}
A Volunteer is able to : 
\begin{itemize} 
	\item Give feedback
	\item Create a new session for initiatives they are assigned to
	\item Keep track of members' attendance to those initiatives
	\item Access restricted information about a user
\end{itemize}
An User is able to : 
\begin{itemize} 
	\item Give feedback
	\item Make request to the Administrator if they wish to change their personal information
\end{itemize}
\end{abstract}
