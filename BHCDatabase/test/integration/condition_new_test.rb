...
class ConditionNewTest < ActionDispatch::IntegrationTest
  ...
  test "valid new condition test" do
    ...
    assert_difference 'Condition.count', 1 do
      post conditions_path, params: { condition: { user_id: @service_user.id, medical_condition_id: @medical_condition_one.name } }
    end
    follow_redirect!
  ...
  test "duplicate new condition test" do
    get new_condition_path, params: { user_id: @service_user.id }
    assert_no_difference 'Condition.count' do
      post conditions_path, params: { condition: { user_id: @service_user.id, medical_condition_id: @medical_condition_two.name } }
    end
    follow_redirect!
    assert_not flash.empty?
    assert_template 'users/show'
